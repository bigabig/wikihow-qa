...
for row in range(rows):
    ...
    title = Data.ix[row,'title']           # title is the column representing the title
    ...
    if len(abstract) < (0.75*len(article)):
        ...
        with open(path+'/'+filename1,'wb') as f:
			...
			data_obj['summary'] = []
			data_obj['summary_string'] = ""
			data_obj['article'] = article
		
            with open('temporaryFile.txt','r') as t:
                for line in t:
                    line=line.lower()
                    if line != "\n" and line != "\t" and line != " ":
						data_obj['summary'].append(line)
						data_obj['summary_string']+=line.strip()+" "
                    

				
			data_obj['summary_string'].strip()
