...
class BertForSequenceClassification(nn.Module):
    ...
    # HIER NICHT CLASSIFIER SONDERN DECODER!
        ...
        for phase in ['train', 'val']:
            ...
            for inputs, sentiment in dataloaders_dict[phase]:
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    # get model output for the input sequence
                    outputs = model(inputs)

                    # WARUM IST DER SOFTMAX LAYER NICHT IM MODEL?
                    # apply softmax on the model output
                    outputs = F.softmax(outputs, dim=1)

                    # Calculate Loss            WAS MACHT DAS?
            ...
            # save model if the loss was better than last best loss
...
# Figure out whether to use the GPU or CPU
...
# input_text = "what is a pug"

# Tokenize Input sentence
# tokenized_input = tokenizer.tokenize(input_text)

# Convert inputs to PyTorch tensors
# tokens_tensor = torch.tensor([tokenizer.convert_tokens_to_ids(tokenized_input)])
