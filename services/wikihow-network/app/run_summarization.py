...
def main_old(unused_argv):
  ...
  FLAGS.mode = 'decode'
  FLAGS.data_path = '../finished_files/chunked/test_*'
  FLAGS.vocab_path = '../finished_files/vocab'
  ...
  if hps.mode == 'train':
    ...
    result = decoder.decode2() # decode indefinitely (unless single_pass=True, in which case deocde the dataset exactly once)
    print("my result coming!! NICE")
    print(result)
  else:
...
def do_decode(abstract, article):
  global model
  global vocab
  global hps
  global decoder
  ...
  batcher = Batcher(FLAGS.data_path, vocab, hps, single_pass=FLAGS.single_pass, abstract=abstract, article=article)
  result = decoder.decode2(batcher)
  return result
