...
def example_generator2(abstract, article):
  # process article
  ...
  out_article = ' '.join(article_tokens)
  out_article = out_article.strip()
  print(out_article)
  ...
  for sentence in abstract_sentences:
    ...
    abstract_sentence = ' '.join(sentence_tokens)

    out_abstract += " %s %s %s" % (SENTENCE_START, abstract_sentence.lower(), SENTENCE_END)
  out_abstract = out_abstract.strip()
  print(out_abstract)

  # Write to tf.Example
  tf_example = example_pb2.Example()
  tf_example.features.feature['article'].bytes_list.value.extend([out_article.encode('utf-8')])
  tf_example.features.feature['abstract'].bytes_list.value.extend([out_abstract.encode('utf-8')])
